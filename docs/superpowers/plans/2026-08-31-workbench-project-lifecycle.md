...
- [ ] **Step 1: 写失败测试**(覆盖:404 / 路径逃逸 400 / 双形态删除 / 级联完整 / running 对话行也被删 / repoError 不回滚 / recap 清空归零水位 / recap 超长 400)。用真 `node:sqlite` 临时库 + `mkdtemp` 临时 workbenchDir + 真 git init 不必要(repo 目录用假文件树即可,`fs.mkdirSync(projectRepoPath(...), {recursive:true})` + 放一个哨兵文件,断言删除后目录消失)。路径逃逸用例:手工 UPDATE 该行 `repoRoot='../../escape'` → deleteProject 返 400 且文件仍在
- [ ] **Step 2: 跑测试确认失败** — `node --test server/workbench-projects.test.mjs`
- [ ] **Step 3: 实现**(顶部 `import { rmSync } from 'node:fs'`、`import { resolve, join } from 'node:path'`;事务 `db.exec('BEGIN'/'COMMIT'/'ROLLBACK')` 同对话删除写法)
- [ ] **Step 4: 跑测试全绿**
- [ ] **Step 5: Commit** `feat(workbench): 项目删除数据层——级联事务+repoRoot 双形态+路径逃逸拒绝+recap 人工写`
...
- [ ] **Step 1: 写失败测试**(确认名不符 400 / 403 / 404 / 成功删除后 GET 列表不再含它、其对话 GET 404 / PATCH 改名生效 / PATCH recap 空串清空)
- [ ] **Step 2: 跑测试确认失败**(DELETE/PATCH 现为 404——门的行为)
- [ ] **Step 3: 实现**(路由插在既有 `seg[1] === 'cluster'` 分支旁;DELETE 里先查对话列表拿 running 集合与计数再调数据层)
- [ ] **Step 4: 全绿 + 回归** `node --test server/routes/`
- [ ] **Step 5: Commit** `feat(workbench): 项目删除/重命名/recap 人工通道路由——owner 收严+确认名+审计`
...
- [ ] **Step 1: 写失败测试**(summarize:mock db 捕获拼装的 summarizer 指令含约束语句;agent:注入断言更新为带 caveat 的新字面)
- [ ] **Step 2: 确认失败** → **Step 3: 实现** → **Step 4: 全绿**(回归 `node --test server/workbench-summarize.test.mjs server/workbench-agent.test.mjs`)
- [ ] **Step 5: Commit** `fix(workbench): 摘要器加固——禁止瞬时能力结论入记忆+注入头标注以本轮实际工具为准(毒记忆事故)`
...
- [ ] **Step 1: 写失败 vitest** → **Step 2: 红** → **Step 3: 实现 client + 视图 + i18n** → **Step 4: `npx vitest run src/views/__tests__/WorkbenchList.lifecycle.test.js` 绿 + `npm run i18n:check` 绿**
- [ ] **Step 5: Commit** `feat(workbench): 项目列表删除/重命名——确认名防误删+行内重命名`
...
- [ ] **Step 1: 写失败 vitest**(mock workbenchApi.updateProject;断言编辑保存调用与清空调用 `recap:''`)→ **Step 2: 红** → **Step 3: 实现 + i18n** → **Step 4: vitest 绿 + i18n:check 绿**
- [ ] **Step 5: Commit** `feat(workbench): 项目记忆卡编辑/清空——人工纠偏通道`
