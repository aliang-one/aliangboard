...
5a. 行 ⋮ 菜单开着时点浮动终端窗:第一次点击先关菜单(遮罩层级行为),二次操作浮窗——确认可接受
6. 桌面/iPad:以上全部无观感变化——零回归

## 13. 顶栏手机档系统化处理(Wave 4 修订,2026-09-02)

**诊断**:375px 视口下顶栏实际内容宽 ≈538px(汉堡36+搜索36+集群chip~140+ns chip~140+工作台胶囊~110+刷新36+头像40+gaps)——双 chip `shrink-0` 双行徽标+带文字胶囊结构性溢出,挤压/裁切即用户所见「图标没处理好」。

### 13.1 手机顶栏新布局(<640)

`[汉堡] [上下文胶囊(ns主/集群副)] (弹性空间) [工作台图标钮] [搜索] [刷新] [头像]`

- **单颗上下文胶囊**替代集群/ns 双 chip:`folder` 图标 + ns 名(主文本,粗体 truncate,弹性 max-w)+ 集群名(副行 text-[10px] truncate)+ `expand_more`;点击弹 **ns 底部选择器**(nsPanel 手机档 bottom sheet 化:Teleport fixed 贴底/rounded-t-2xl/max-h-70vh/overflow-y-auto/safe-area 底垫,复用 DropdownMenu 配方)。ns 高频一跳直达。
- **集群切换进抽屉**:抽屉(SideNavBar)顶部加「当前集群」小节——当前集群名 chip(点击弹 clusterPanel 手机档 bottom sheet,同款样式);桌面/iPad 抽屉不存在,无此节。
- **工作台收成图标钮**:`WorkbenchEntryPill` 手机档渲染为 40px 图标钮(workspaces 图标,活跃态高亮,未读/活跃角标收敛为小圆点),点击仍直达 /workbench;悬停概览面板仅 ≥640。实现于 pill 组件内部(isPhone 分支),消费方零改动。
- **桌面/iPad 零回归**:≥640 双 chip/完整胶囊/锚定下拉全部现状。

### 13.2 交互细节

- 胶囊/nsPanel/clusterPanel 的手机 bottom sheet 均带遮罩(`Z.popover - 1`)与 `@click.stop` 防冒泡(既有配方)。
- 上下文胶囊点击仅开 ns 选择;集群切换走抽屉(低频两跳可接受,裁决:上下文常显价值 > 单跳集群)。
- 手机档 nspanel 底部选择器内搜索输入 autofocus。

### 13.3 测试

- vitest:手机档胶囊在场且双 chip 不渲染/胶囊文本=ns 名+集群名/工作台图标钮在场且 pill 文本不在/nsPanel 手机档 bottom sheet 类/抽屉集群节在场;桌面档全部现状断言。
- 真机手测(§14):375/390/430 明暗——胶囊 truncate 长名/bottom sheet 滚动/工作台图标钮直达/桌面零回归。

## 14. Wave 4(顶栏)真机手测清单

1. 顶栏一行不溢出:汉堡+胶囊+三钮+头像在 375px 内,胶囊长 ns 名/集群名 truncate
2. 胶囊点击弹 ns 底部选择器:搜索/选择/点遮罩关;选择后胶囊即时更新
3. 抽屉顶部当前集群 chip:点击弹集群底部选择器;切换后全局上下文更新
4. 工作台图标钮:直达工作台;活跃态高亮可见
5. 桌面/iPad:双 chip+完整工作台胶囊+锚定下拉——零回归
