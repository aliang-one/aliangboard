...
// 分层体系（顺序即展示顺序；microservice 含子层）
export const LAYER_TAXONOMY = [
  { key: 'presentation', label: '展现层', icon: 'desktop_windows', desc: '前端 / UI / 静态站点' },
  { key: 'gateway', label: '网关', icon: 'alt_route', desc: 'API 网关 / 入口代理 / Ingress 控制器' },
  {
    key: 'microservice', label: '微服务层', icon: 'hub', desc: '微服务应用', children: [
      { key: 'microservice/business', label: '业务层', icon: 'storefront', desc: '核心业务服务' },
      { key: 'microservice/support', label: '支持服务', icon: 'support_agent', desc: '鉴权 / 配置 / 调度等支撑服务' },
      { key: 'microservice/misc', label: '杂项', icon: 'widgets', desc: '批处理任务 / 其它' },
    ],
  },
  { key: 'middleware', label: '中间件', icon: 'sync_alt', desc: '消息队列 / 缓存 / 注册中心' },
  { key: 'persistence', label: '持久层', icon: 'database', desc: '关系型 / 文档型 / 搜索数据库' },
  { key: 'storage', label: '存储', icon: 'storage', desc: '对象 / 文件存储' },
  { key: 'monitoring', label: '监控层', icon: 'monitoring', desc: '指标 / 日志 / 链路追踪' },
  { key: 'unclassified', label: '未分类', icon: 'label', desc: '未能自动识别——可用 label layer.aliangboard.io 显式归类' },
]
...
export function classifyResource(res) {
  ...
  const explicit = labels['layer.aliangboard.io'] || ann['layer.aliangboard.io'] || labels['layer'] || ann['aliangboard.io/layer']
