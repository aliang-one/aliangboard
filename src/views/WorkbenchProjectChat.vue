...
<template>
  <section class="animate-fade-in h-full flex flex-col gap-md min-h-0">
    <div class="shrink-0 flex items-center gap-sm">
      ...
      <span class="text-body-sm text-on-surface-variant">· {{ t('workbench.chat.title') }}</span>
      <button v-if="turns.length" @click="clearChat" class="ml-auto flex items-center gap-xs px-md py-xs border border-outline-variant rounded-lg text-body-sm hover:bg-surface-container"><span class="material-symbols-outlined text-sm">delete_sweep</span>{{ t('workbench.chat.clearChat') }}</button>
    </div>
    <p class="shrink-0 text-body-sm text-on-surface-variant -mt-sm">{{ t('workbench.chat.subtitle') }}</p>
    ...
    <div ref="scrollEl" class="flex-1 min-h-0 overflow-y-auto flex flex-col gap-md pr-xs">
      <div v-if="!turns.length" class="flex-1 flex items-center justify-center">
        <div class="text-center max-w-md">
          ...
          <p class="text-body-md text-on-surface-variant mt-md">{{ t('workbench.chat.hint') }}</p>
        ...
        <div v-else class="w-full max-w-[92%] flex flex-col gap-sm">
          <details v-if="t.trace && t.trace.length" open class="bg-surface-container-low border border-outline-variant rounded-lg">
            <summary class="cursor-pointer px-md py-sm text-body-sm text-on-surface-variant select-none flex items-center gap-xs">
              <span class="material-symbols-outlined text-base">account_tree</span> {{ t('workbench.chat.toolTrace', { n: toolCount(t.trace) }) }}
            </summary>
          ...
          <div v-if="t.status === 'thinking' && !(t.trace && t.trace.length)" class="flex items-center gap-sm text-body-sm text-on-surface-variant bg-surface-container-lowest border border-outline-variant rounded-2xl rounded-tl-sm px-md py-sm">
            <span class="material-symbols-outlined animate-spin text-base">progress_activity</span> {{ t('workbench.chat.thinking') }}
          </div>
          <div v-else-if="t.status === 'pending_approval'" class="flex items-center gap-xs text-body-sm text-status-warning bg-status-warning/10 border border-status-warning/30 rounded-2xl rounded-tl-sm px-md py-sm">
            <span class="material-symbols-outlined text-base">pending_actions</span> {{ t('workbench.chat.pendingApproval') }}
          </div>
          ...
          <template v-else-if="t.status === 'done'">
            <div class="text-body-md text-on-surface whitespace-pre-wrap break-words bg-surface-container-lowest border border-outline-variant rounded-2xl rounded-tl-sm px-md py-sm">{{ t.content || t('workbench.chat.noAnswer') }}</div>
            <div class="flex items-center gap-md text-body-xs text-on-surface-variant px-xs"><span>{{ t('workbench.chat.stepsTaken', { n: t.steps }) }}</span><span v-if="t.truncated" class="text-status-warning">{{ t('workbench.chat.maxStepsWarning') }}</span></div>
          </template>
        </div>
      </div>
    </div>
    ...
    <div class="shrink-0 flex items-end gap-sm bg-surface-container-lowest border border-outline-variant rounded-xl p-sm">
      <textarea v-model="input" @keydown="onKeydown" :disabled="sending || !!pendingApproval" rows="2" :placeholder="t('workbench.chat.userMessage')" class="flex-1 bg-transparent resize-none outline-none text-body-sm px-sm py-sm max-h-40"></textarea>
      <button @click="send" :disabled="sending || !input.trim() || !!pendingApproval" class="shrink-0 flex items-center gap-xs px-md py-sm bg-primary text-on-primary rounded-lg font-semibold disabled:opacity-40"><span class="material-symbols-outlined text-base">send</span> {{ t('workbench.chat.send') }}</button>
    </div>
