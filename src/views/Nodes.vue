...
async function sync() {
  ...
  syncing.value = true
  try { await store.hydrateCoreResources(); notify('success', t('nodes.synced')) }
  ...
  <section class="animate-fade-in">
    <div class="flex justify-between items-end mb-md">
      <div>
        ...
        <p class="text-on-surface-variant text-body-sm mt-xs">{{ $t('nodes.subtitle') }} {{ $t('nodes.subtitleDetail', { healthy: store.healthyNodes, total: store.totalNodes }) }}</p>
      </div>
    ...
    <EmptyState v-if="!filtered.length" icon="dns" :title="$t('nodes.noNodesTitle')" :description="$t('nodes.noNodesDesc')" />
