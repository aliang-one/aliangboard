...
const podCpuRefLines = computed(() => {
  const r = []
  if (podRes.value.cpuReq) r.push({ label: 'requests', value: podRes.value.cpuReq, color: 'secondary' })
  if (podRes.value.cpuLim) r.push({ label: 'limits', value: podRes.value.cpuLim, color: 'error' })
  return r
})
const podMemRefLines = computed(() => {
  const r = []
  if (podRes.value.memReq) r.push({ label: 'requests', value: podRes.value.memReq, color: 'secondary' })
  if (podRes.value.memLim) r.push({ label: 'limits', value: podRes.value.memLim, color: 'error' })
  return r
})
      ...
      <div v-if="isRolloutType" class="flex flex-col gap-md">
        ...
        <section class="rounded-xl border border-outline-variant p-md bg-surface-container-lowest flex flex-col gap-sm">
          ...
          <div v-if="editForm.securityContext.enabled" class="grid grid-cols-3 gap-xs">
            ...
            <div><label class="text-xs font-medium text-on-surface-variant block mb-xs">{{ $t('workload.edit.capabilitiesAdd') }}</label><input v-model="editForm.securityContext.addCaps" class="w-full bg-surface-container-low border border-outline-variant rounded-md px-sm py-sm text-xs font-mono focus:ring-2 focus:ring-primary/20 focus:border-primary transition-colors" :placeholder="$t('workload.edit.capabilitiesAddPlaceholder')" /></div>
        ...
        <section class="rounded-xl border border-outline-variant p-md bg-surface-container-lowest flex flex-col gap-sm">
          ...
          <div class="flex items-center justify-between"><span class="text-xs font-semibold text-on-surface-variant">{{ $t('workload.edit.nodeSelector') }}</span><button @click="editForm.nodeSelectors.push({ key: '', value: '' })" class="flex items-center gap-0.5 text-xs font-medium text-primary hover:bg-primary-container/10 rounded px-xs py-0.5 transition-colors"><span class="material-symbols-outlined text-sm">add</span>{{ $t('workload.edit.add') }}</button></div>
