      ...
      <div class="w-56 shrink-0 bg-surface-container-lowest border border-outline-variant rounded-xl overflow-hidden flex flex-col">
        ...
        <div class="flex-1 overflow-y-auto max-h-[60vh]">
          <button v-for="([key, val], idx) in dataEntries" :key="idx"
            ...
            <span @click.stop="deleteKey(key); selectedKey = dataEntries.find(([k]) => k !== key)?.[0] || ''" class="opacity-0 group-hover:opacity-100 max-sm:opacity-100 p-0.5 text-on-surface-variant hover:text-error rounded transition-opacity shrink-0 cursor-pointer max-sm:p-2 max-sm:-m-2" :title="$t('ns.cmDetail.delete')">
