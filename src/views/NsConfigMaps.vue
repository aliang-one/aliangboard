...
<template>
  <section class="animate-fade-in">
    ...
    <DataTable v-if="filtered.length" :headers="headers" :rows="paginated" column-key="nsConfigMaps" selectable v-model:selection="selected" row-key="name" @row-click="goDetail">
      <template #name="{ row }">
        <div class="flex items-center gap-sm">
          <span class="material-symbols-outlined text-secondary text-sm shrink-0">description</span>
    ...
    <div v-else class="bg-surface-container-lowest border border-outline-variant rounded-xl p-md text-center">
      ...
      <button v-if="search" @click="search = ''" class="mt-xs px-3 py-1.5 max-sm:min-h-[40px] border border-outline-variant rounded-lg text-body-sm font-medium hover:bg-surface-container-high">{{ t('ns.configmaps.clearSearch') }}</button>
      <button v-else data-testid="open-create" @click="showCreateModal = true" class="mt-xs px-3 py-1.5 max-sm:min-h-[40px] bg-primary text-on-primary rounded-lg text-body-sm font-semibold hover:opacity-90">{{ t('ns.configmaps.createShort') }}</button>
    </div>
  </section>
