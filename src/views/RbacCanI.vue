  ...
  <div class="animate-fade-in">
    ...
    <div class="flex items-center justify-between mt-sm mb-md">
      <div class="flex items-center gap-md">
        ...
        <div>
          <h1 class="text-headline-lg text-on-surface font-bold">权限模拟</h1>
          <p class="text-body-xs text-on-surface-variant mt-xs">kubectl auth can-i — 基于 RBAC 规则推演某 subject 能否执行指定操作</p>
        </div>
      </div>
    </div>
      ...
      <div class="lg:col-span-7">
        <div class="rounded-xl overflow-hidden bg-surface-container-lowest border border-outline-variant">
          ...
          <div class="grid grid-cols-2 gap-sm">
            <div>
              <label class="text-body-xs text-on-surface-variant block mb-xs">Subject Kind</label>
            ...
            <div>
              <label class="text-body-xs text-on-surface-variant block mb-xs">Subject Name</label>
            ...
            <div>
              <label class="text-body-xs text-on-surface-variant block mb-xs">Verb</label>
            ...
            <div>
              <label class="text-body-xs text-on-surface-variant block mb-xs">Resource</label>
            ...
            <div class="col-span-2">
              <label class="text-body-xs text-on-surface-variant block mb-xs">Namespace（可选，留空表示任意/集群级）</label>
      ...
      <div class="lg:col-span-5">
        <div class="rounded-xl overflow-hidden bg-surface-container-lowest border border-outline-variant sticky top-md">
          ...
          <div v-if="!result" class="py-md text-center text-on-surface-variant">
            ...
            <p class="mt-xs text-body-xs">填写条件后点击「规则推演」</p>
          </div>
          <div v-else>
            <div class="flex items-center gap-sm p-md rounded-lg mb-sm"
              ...
              <div>
                ...
                <p class="text-body-xs font-mono">{{ subjectKind }} <span class="text-on-surface">{{ subjectName || '*' }}</span> 对 <span class="text-on-surface font-mono">{{ resource }}</span> 执行 <span class="text-on-surface font-mono">{{ verb }}</span></p>
              </div>
            </div>
            <div v-if="result.matchedBy" class="text-body-sm">
              <p class="text-body-xs text-on-surface-variant mb-xs">命中规则</p>
              <p class="font-mono text-code-xs text-on-surface bg-surface-container-low p-sm rounded-lg break-all">{{ result.matchedBy }}</p>
              <div v-if="result.rule" class="mt-sm">
                <p class="text-body-xs text-on-surface-variant mb-xs">Rule</p>
                <pre class="font-mono text-code-xs text-on-surface-variant bg-surface-container-low p-sm rounded-lg overflow-auto">{{ JSON.stringify(result.rule, null, 2) }}</pre>
              </div>
            </div>
            <div v-else class="text-body-xs text-on-surface-variant">
        ...
        <div class="rounded-xl overflow-hidden bg-surface-container-lowest border border-outline-variant mt-md">
          <div class="px-md py-2.5 border-b border-outline-variant/50 flex items-center justify-between">
            ...
            <span class="text-body-xs text-on-surface-variant">SelfSubjectAccessReview</span>
          </div>
          <div class="p-md">
          <div v-if="!serverResult" class="py-sm text-center text-on-surface-variant text-body-xs">
            点击「服务端 can-i」查询当前登录用户的服务端判定（kubectl auth can-i 的服务端语义）。
          </div>
          <div v-else-if="!serverResult.ok" class="text-body-xs text-error">{{ serverResult.error }}</div>
          <div v-else>
            <div class="flex items-center gap-sm p-sm rounded-lg mb-sm"
              ...
              <div>
                ...
                <p class="text-body-xs font-mono">当前用户 对 <span class="text-on-surface font-mono">{{ resource }}</span> 执行 <span class="text-on-surface font-mono">{{ verb }}</span></p>
              </div>
            </div>
            <p v-if="serverResult.reason" class="text-body-xs text-on-surface-variant">原因：<span class="font-mono text-code-xs">{{ serverResult.reason }}</span></p>
