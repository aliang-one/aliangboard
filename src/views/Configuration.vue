...
const cmQ = useResourceList({ key: ['cluster', cid.value, 'configmaps'], fetcher: () => store.fetchConfigMaps(), options: { refetchInterval: 30000 } })
const secQ = useResourceList({ key: ['cluster', cid.value, 'secrets'], fetcher: () => store.fetchSecrets(), options: { refetchInterval: 30000 } })
const rqQ = useResourceList({ key: ['cluster', cid.value, 'resourcequotas'], fetcher: () => store.fetchResourceQuotas(), options: { refetchInterval: 30000 } })
const lrQ = useResourceList({ key: ['cluster', cid.value, 'limitranges'], fetcher: () => store.fetchLimitRanges(), options: { refetchInterval: 30000 } })
const hpaQ = useResourceList({ key: ['cluster', cid.value, 'hpas'], fetcher: () => store.fetchHPAs(), options: { refetchInterval: 30000 } })
