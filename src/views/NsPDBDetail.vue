...
<template>
    ...
    <div v-if="activeTab === 'overview'" class="flex flex-col gap-lg">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-lg">
        ...
        <div class="bg-surface-container-lowest border border-outline-variant rounded-xl shadow-card p-lg">
          <div class="flex items-center gap-sm mb-md">
            ...
            <h3 class="text-headline-sm text-on-surface">{{ t('ns.pdbDetail.allowedDisruptions') }}</h3>
          </div>
        ...
        <div class="bg-surface-container-lowest border border-outline-variant rounded-xl shadow-card p-lg">
          <div class="flex items-center gap-sm mb-md">
            ...
            <h3 class="text-headline-sm text-on-surface">{{ t('ns.pdbDetail.healthStatus') }}</h3>
          </div>
          <div class="flex items-end gap-md">
            <div class="flex flex-col">
              <span class="text-label-caps text-on-surface-variant">{{ t('ns.pdbDetail.currentHealthy') }}</span>
            ...
            <div class="flex flex-col">
              <span class="text-label-caps text-on-surface-variant">{{ t('ns.pdbDetail.desiredHealthy') }}</span>
          ...
          <div v-if="!isHealthy" class="mt-sm flex items-center gap-xs px-md py-sm bg-error-container/10 border border-error/20 rounded-lg">
            ...
            <span class="text-body-sm text-error font-medium">{{ t('ns.pdbDetail.healthWarning') }}</span>
          </div>
          <p v-else class="text-body-sm text-on-surface-variant mt-sm">{{ t('ns.pdbDetail.healthNormal') }}</p>
        </div>
      </div>
      ...
      <div class="bg-surface-container-lowest border border-outline-variant rounded-xl shadow-card overflow-hidden">
        <div class="px-lg py-md border-b border-outline-variant bg-surface-container-low">
          <h3 class="text-headline-sm text-on-surface">{{ t('ns.pdbDetail.constraintInfo') }}</h3>
        </div>
        <div class="divide-y divide-outline-variant/30">
          ...
          <div class="px-lg py-md flex items-center justify-between">
            <div class="flex items-center gap-sm">
              ...
              <span class="text-body-md text-on-surface-variant">{{ t('ns.pdbDetail.constraintType') }}</span>
            </div>
            <div class="flex items-center gap-sm">
          ...
          <div class="px-lg py-md flex items-center justify-between gap-md">
            <div class="flex items-center gap-sm shrink-0">
              ...
              <span class="text-body-md text-on-surface-variant">{{ t('ns.pdbDetail.selector') }}</span>
            </div>
  ...
  <Modal v-model="showEditModal" title="Edit PodDisruptionBudget" width="max-w-lg">
    <div class="flex flex-col gap-md">
      ...
      <div>
        <label class="text-label-caps text-on-surface-variant block mb-xs">{{ t('ns.pdbDetail.valueLabel') }}</label>
        <input v-if="editForm.constraintType === 'minAvailable'" v-model="editForm.minAvailable" class="w-full bg-surface-container-low border border-outline-variant rounded-lg px-md py-sm text-body-md font-mono focus:ring-2 focus:ring-primary" :placeholder="t('ns.pdbDetail.minAvailablePlaceholder')" />
        <input v-else v-model="editForm.maxUnavailable" class="w-full bg-surface-container-low border border-outline-variant rounded-lg px-md py-sm text-body-md font-mono focus:ring-2 focus:ring-primary" :placeholder="t('ns.pdbDetail.maxUnavailablePlaceholder')" />
      </div>
    </div>
    <template #actions>
