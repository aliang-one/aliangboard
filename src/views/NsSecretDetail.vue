  ...
  <div class="animate-fade-in" v-if="secret">
    ...
    <div class="flex flex-wrap items-start justify-between gap-x-sm gap-y-sm mt-sm mb-xl">
      <div class="flex items-center gap-lg min-w-0">
        ...
        <div class="min-w-0">
          <h1 class="text-display-lg text-on-surface min-w-0 max-sm:truncate" :title="secret.name">{{ secret.name }}</h1>
          <div class="flex items-center gap-md mt-xs flex-wrap">
            <span class="px-2.5 py-0.5 rounded-full text-label-caps font-medium border break-all" :class="typeBadge.color">
    ...
    <div v-if="activeTab === 'data'">
      ...
      <div class="bg-surface-container-lowest border border-outline-variant rounded-xl shadow-card overflow-hidden">
        <div class="px-lg py-md border-b border-outline-variant bg-surface-container-low flex items-center justify-between">
          ...
          <button @click="showAddKeyModal = true" class="flex items-center gap-sm px-md py-xs bg-primary text-on-primary rounded-lg text-body-sm font-semibold hover:opacity-90 max-sm:min-h-[40px]">
          ...
          <div v-for="([key, val], idx) in dataEntries" :key="idx" class="px-lg py-md">
            <div class="flex items-center justify-between gap-sm mb-sm">
              <div class="flex items-center gap-sm min-w-0">
                <span class="font-mono text-code-sm text-primary font-semibold truncate" :title="key">{{ key }}</span>
                <button @click="toggleReveal(key)" class="p-xs text-on-surface-variant hover:text-primary rounded-lg shrink-0 relative max-sm:after:absolute max-sm:after:-inset-2 max-sm:after:content-['']" :title="revealedKeys.has(key) ? $t('ns.secretDetail.hide') : $t('ns.secretDetail.reveal')">
              ...
              <div class="flex gap-xs shrink-0">
                <button v-if="editingKey !== key" @click="startEdit(key)" class="p-xs text-on-surface-variant hover:text-primary hover:bg-primary-container/10 rounded-lg relative max-sm:after:absolute max-sm:after:-inset-2 max-sm:after:content-['']">
                  <span class="material-symbols-outlined text-lg">edit</span>
                </button>
                <button @click="deleteKey(key)" class="p-xs text-on-surface-variant hover:text-error hover:bg-error-container/20 rounded-lg relative max-sm:after:absolute max-sm:after:-inset-2 max-sm:after:content-['']">
                  <span class="material-symbols-outlined text-lg">delete</span>
                </button>
              </div>
            </div>
            <div v-if="editingKey === key" class="flex gap-sm">
              <textarea v-model="editValue" class="flex-1 min-w-0 min-h-[80px] resize-y bg-surface-container-low border border-outline-variant rounded-lg px-md py-sm text-body-sm font-mono focus:ring-2 focus:ring-primary"></textarea>
