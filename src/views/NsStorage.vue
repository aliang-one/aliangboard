  ...
  <section class="animate-fade-in">
      ...
      <DataTable :headers="pvcHeaders" :rows="rowsWithUsage" column-key="nsStoragePVC" @row-click="goPVCDetail">
        <template #name="{ row }">
          <div class="flex items-center gap-sm">
            <span class="material-symbols-outlined text-primary text-lg shrink-0">storage</span>
        ...
        <template #actions="{ row }">
          <div class="flex gap-1">
            <button @click.stop="goPVCDetail(row)" class="relative max-sm:after:absolute max-sm:after:-inset-2 max-sm:after:content-[''] p-xs text-on-surface-variant hover:text-primary hover:bg-primary-container/10 rounded-lg"><span class="material-symbols-outlined text-lg">open_in_new</span></button>
    ...
    <div v-if="activeTab === 'storageclass'">
      ...
      <DataTable :headers="scHeaders" :rows="allSCs" column-key="nsStorageSC" @row-click="goSCDetail">
        <template #name="{ row }">
          <div class="flex items-center gap-sm">
            <span class="material-symbols-outlined text-secondary text-lg shrink-0">database</span>
            <span class="font-semibold text-on-surface text-body-md truncate min-w-0" :title="row.name">{{ row.name }}</span>
          </div>
        </template>
