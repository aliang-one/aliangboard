                ...
                <div class="flex-1 min-w-0 flex overflow-hidden rounded-xl">
                  <div class="flex-1 min-w-0 p-md flex flex-col">
                    ...
                    <div class="mt-auto pt-sm grid grid-cols-3 gap-xs">
                      ...
                      <div><p class="text-[10px] text-on-surface-variant/50 uppercase">{{ t('monitoring.live') }}</p><p class="text-body-sm text-on-surface-variant">{{ it.dep.age }}</p></div>
    ...
    <Teleport to="body">
      <div v-if="hover" class="fixed w-80 rounded-xl bg-surface-container-lowest border border-outline-variant shadow-2xl overflow-hidden animate-fade-in" :style="{ top: hover.top + 'px', right: hover.right + 'px', zIndex: Z.modal }" @mouseenter="onPopoverEnter" @mouseleave="onLeaveAssoc">
        ...
        <div v-if="hover.type === 'svc'" class="p-sm flex flex-col gap-sm max-h-72 overflow-y-auto">
          <div v-for="s in hover.items" :key="s.name" class="flex flex-col gap-0.5"><div class="flex items-center gap-xs"><span class="font-mono text-xs font-semibold text-on-surface truncate">{{ s.name }}</span><span class="ml-auto shrink-0 px-1 py-0.5 rounded bg-surface-container text-[10px] text-on-surface-variant">{{ s.type }}</span></div><p class="font-mono text-[11px] text-on-surface-variant truncate"><span class="text-on-surface-variant/50">ClusterIP</span> <span class="text-on-surface">{{ s.clusterIP }}</span></p><p v-if="s.externalIP && s.externalIP !== '-'" class="font-mono text-[11px] text-on-surface-variant truncate"><span class="text-on-surface-variant/50">{{ t('ns.services.thExternalIp') }}</span> <span class="text-secondary">{{ s.externalIP }}</span></p><p class="font-mono text-[11px] truncate"><span class="text-on-surface-variant/50">{{ t('ns.services.thPorts') }}</span> <span class="text-primary">{{ s.ports || '—' }}</span></p><p v-if="fmtSelector(s.selector)" class="text-[11px] text-on-surface-variant truncate"><span class="text-on-surface-variant/50">{{ t('ns.services.thSelector') }}</span> {{ fmtSelector(s.selector) }}</p><p class="text-[11px] text-on-surface-variant/50">{{ t('ns.namespaceOverview.created') }} {{ s.age }}</p></div>
        </div>
        <div v-else class="p-sm flex flex-col gap-sm max-h-72 overflow-y-auto">
          <div v-for="(r, i) in hover.items" :key="r.ingName + i" class="flex flex-col gap-0.5"><div class="flex items-center gap-xs"><span class="font-mono text-xs font-semibold text-secondary truncate">{{ r.host }}<span class="text-on-surface-variant font-normal">{{ r.path }}</span></span><span v-if="r.tls" class="ml-auto shrink-0 px-1 py-0.5 rounded bg-status-running/10 text-status-running text-[10px] flex items-center gap-0.5"><span class="material-symbols-outlined" style="font-size:11px">lock</span>TLS</span></div><p class="font-mono text-[11px] truncate"><span class="text-on-surface-variant/50">{{ t('ns.namespaceOverview.backend') }}</span> <span class="text-primary">{{ r.serviceName }}{{ r.servicePort ? ':' + r.servicePort : '' }}</span></p><p class="text-[11px] text-on-surface-variant truncate"><span class="text-on-surface-variant/50">Ingress</span> {{ r.ingName }}<span v-if="r.className"> · {{ r.className }}</span></p><p v-if="r.tlsSecret" class="text-[11px] text-on-surface-variant truncate"><span class="text-on-surface-variant/50">{{ t('ns.ingressDetail.tlsSecret') }}</span> {{ r.tlsSecret }}</p><p class="text-[11px] text-on-surface-variant/50">{{ t('ns.namespaceOverview.created') }} {{ r.age }}</p></div>
        </div>
