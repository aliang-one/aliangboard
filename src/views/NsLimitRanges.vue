    ...
    <div v-if="store.nsLimitRanges.length" class="bg-surface-container-lowest border border-outline-variant rounded-xl shadow-card overflow-hidden">
      <table class="w-full text-left border-collapse">
        ...
        <tbody class="divide-y divide-outline-variant/30">
          <tr v-for="row in store.nsLimitRanges" :key="row.name" class="hover:bg-surface-container-low/50 cursor-pointer transition-colors" @click="router.push({ name: 'NsLimitRangeDetail', params: { namespace: route.params.namespace, name: row.name } })">
