...
test('品牌瓷砖:workspaces 图标装进 primary→tertiary 渐变圆角瓷砖(on-primary 图标亮暗自翻转),桌面/手机档都在场', async () => {
  ...
  w = mountPill(); await flushPromises()
  expect(w.find('[data-test="pill-tile"]').exists()).toBe(true)
  w.unmount(); spy.mockRestore()
})

test('sheen 高光扫过:仅 /workbench* 在场,必带 bg-no-repeat(repeat 会把 200% 白带绕回瓷砖常驻洗白)与 motion-reduce:hidden(animate-none 会定格在 0% 洗白帧)', async () => {
  mocks.summary.mockReturnValue(new Promise(() => {}))
  let w = mountPill(); await flushPromises()
  expect(w.find('[data-test="pill-sheen"]').exists()).toBe(false)   // idle 无 sheen
  w.unmount()
  mocks.path = '/workbench/p1'
  w = mountPill(); await flushPromises()
  const sheen = w.find('[data-test="pill-sheen"]')
  expect(sheen.exists()).toBe(true)
  expect(sheen.classes()).toContain('animate-sheen')
  expect(sheen.classes()).toContain('bg-no-repeat')
  expect(sheen.classes()).toContain('motion-reduce:hidden')
  expect(sheen.classes()).not.toContain('motion-reduce:animate-none')
  w.unmount()
})

test('舷形悬停:按钮轮廓镜像舷板外弧(tl 30 / bl 12),填充层 scale-x 扫入;触屏整层隐藏(粘滞 hover 防线,不属 V4 动作语义)', async () => {
  mocks.summary.mockReturnValue(new Promise(() => {}))
  const w = mountPill(); await flushPromises()
  const btn = w.find('[data-test="wb-pill"] button')
  expect(btn.classes()).toContain('rounded-tl-[30px]')
  expect(btn.classes()).toContain('rounded-bl-[12px]')
  // 手机档 40px 方钮不吃桌面舷形大弧(30px 占边长 75% 成花瓣形)
  expect(btn.classes()).toContain('max-sm:rounded-[14px]')
  const fill = btn.find('[data-test="pill-hoverfill"]')
  expect(fill.exists()).toBe(true)
  // 方向性扫入(transform 而非 opacity):overflow-guard V4 管「hover 显隐动作」须手机常显,
  // 装饰性填充正确语义是触屏不存在——scale-x-0 原点左,300ms 扫到 scale-x-100
  expect(fill.classes()).toContain('scale-x-0')
  expect(fill.classes()).toContain('group-hover:scale-x-100')
  expect(fill.classes()).toContain('origin-left')
  expect(fill.classes()).toContain('max-sm:hidden')
  expect(fill.classes()).toContain('motion-reduce:transition-none')
  expect(fill.classes().join(' ')).toContain('from-primary/')
})

test('灯珠(xl+ 统计条):项目中性常亮/运行绿(无待批才呼吸)/待批红脉冲独占注意力,归零全静止', async () => {
  // 运行+待批并存:红 ping 是行动级信号,绿 breathe 让位(只着色不动画)
  mocks.summary.mockResolvedValue(SUMMARY({ totals: { projects: 5, runningConvs: 1, pendingApprovals: 2, sshSessions: 0 } }))
  let w = mountPill(); await flushPromises()
  let stats = w.find('[data-test="pill-stats"]')
  expect(stats.find('[data-test="pill-dot-projects"]').classes()).toContain('bg-outline-variant/60')
  let runDot = stats.find('[data-test="pill-dot-running"]')
  expect(runDot.classes()).toContain('bg-status-running')
  expect(runDot.classes()).not.toContain('animate-breathe')
  const ping = stats.find('[data-test="pill-ping-pending"]')
  expect(ping.exists()).toBe(true)
  expect(ping.classes()).toContain('animate-ping')
  expect(ping.classes()).toContain('motion-reduce:animate-none')
  expect(stats.find('[data-test="pill-dot-pending"]').classes()).toContain('bg-error')
  w.unmount()

  // 仅运行:绿 dot 呼吸;待批归零 ping 摘除、灯珠转中性
  mocks.summary.mockResolvedValue(SUMMARY({ totals: { projects: 5, runningConvs: 1, pendingApprovals: 0, sshSessions: 0 } }))
  w = mountPill(); await flushPromises()
  stats = w.find('[data-test="pill-stats"]')
  runDot = stats.find('[data-test="pill-dot-running"]')
  expect(runDot.classes()).toContain('bg-status-running')
  expect(runDot.classes()).toContain('animate-breathe')
  expect(runDot.classes()).toContain('motion-reduce:animate-none')
  expect(stats.find('[data-test="pill-ping-pending"]').exists()).toBe(false)
  expect(stats.find('[data-test="pill-dot-pending"]').classes()).toContain('bg-outline-variant/60')
  w.unmount()

  // 归零:灯珠转中性且无任何 animate-* 类(顶栏无常驻空转动画)
  mocks.summary.mockResolvedValue(SUMMARY({ totals: { projects: 5, runningConvs: 0, pendingApprovals: 0, sshSessions: 0 } }))
  w = mountPill(); await flushPromises()
  stats = w.find('[data-test="pill-stats"]')
  const idleRun = stats.find('[data-test="pill-dot-running"]')
  expect(idleRun.classes()).toContain('bg-outline-variant/60')
  expect(idleRun.classes().some(c => c.startsWith('animate-'))).toBe(false)
  expect(stats.find('[data-test="pill-ping-pending"]').exists()).toBe(false)
  expect(stats.find('[data-test="pill-dot-pending"]').classes()).toContain('bg-outline-variant/60')
  w.unmount()
})
