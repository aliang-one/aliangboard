...
<template>
  ...
  >
    ...
    <div class="flex items-center gap-sm">
      ...
      <slot name="actions" />
      <button v-if="showTerminal" @click.stop="openTerm" :disabled="!canExec" :title="canExec ? t('component.podCard.terminalTitle') : t('component.podCard.terminalDisabled')" class="p-0.5 rounded hover:bg-primary/10 text-on-surface-variant/50 hover:text-primary transition-colors shrink-0 disabled:opacity-30 disabled:cursor-not-allowed disabled:hover:bg-transparent disabled:hover:text-on-surface-variant/50 relative max-sm:after:absolute max-sm:after:-inset-2 max-sm:after:content-['']"><span class="material-symbols-outlined text-sm">terminal</span></button>
      <button v-if="showFiles" @click.stop="openFiles" :disabled="!canExec" :title="canExec ? t('component.podCard.filesTitle') : t('component.podCard.filesDisabled')" class="p-0.5 rounded hover:bg-primary/10 text-on-surface-variant/50 hover:text-primary transition-colors shrink-0 disabled:opacity-30 disabled:cursor-not-allowed disabled:hover:bg-transparent disabled:hover:text-on-surface-variant/50 relative max-sm:after:absolute max-sm:after:-inset-2 max-sm:after:content-['']"><span class="material-symbols-outlined text-sm">folder_open</span></button>
      <button v-if="showLogs" @click.stop="openLogs" :title="t('component.podCard.logsTitle')" data-testid="podcard-logs" class="p-0.5 rounded hover:bg-primary/10 text-on-surface-variant/50 hover:text-primary transition-colors shrink-0 relative max-sm:after:absolute max-sm:after:-inset-2 max-sm:after:content-['']"><span class="material-symbols-outlined text-sm">subject</span></button>
      <button v-if="showDelete" @click.stop="emit('delete', pod)" class="p-0.5 rounded hover:bg-error/10 text-on-surface-variant/50 hover:text-error transition-colors shrink-0 relative max-sm:after:absolute max-sm:after:-inset-2 max-sm:after:content-['']" :title="t('component.podCard.deleteTitle')"><span class="material-symbols-outlined text-sm">delete</span></button>
    </div>
