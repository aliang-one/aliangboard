...
// 输入即清该字段错误 / 切凭据方式清全部(与全站表单校验清扫 179a3ad 的 UX 对齐)
function clearError(key) { delete errors.value[key] }
function resetErrors() { errors.value = {} }
</script>

<template>
  <div class="flex flex-col gap-md">
    <div>
      <label class="text-body-xs text-on-surface-variant block mb-xs">{{ t('admin.clusters.clusterName') }} <span class="text-error">*</span></label>
      <input v-model="form.name" data-testid="cluster-form-name" :class="['w-full bg-surface-container-low border rounded-lg px-md py-sm text-body-sm font-mono', errors.name ? 'border-error' : 'border-outline-variant']" placeholder="prod-cluster" @input="clearError('name')" />
      <p v-if="errors.name" data-testid="cluster-form-error-name" class="text-body-xs text-error mt-xs">{{ errors.name }}</p>
    </div>
    <!-- 凭据方式切换 -->
    <div class="flex gap-xs">
      <button v-for="m in [{k:'kubeconfig',l:t('admin.clusters.authKubeconfig')},{k:'token',l:t('admin.clusters.authToken')},{k:'basic',l:t('admin.clusters.authBasic')}]" :key="m.k" :data-testid="`cluster-form-auth-${m.k}`" @click="form.authMethod = m.k; resetErrors()"
    ...
    <div v-if="form.authMethod === 'kubeconfig'">
      ...
      <textarea v-model="form.kubeconfig" data-testid="cluster-form-kubeconfig" rows="8" :class="['w-full bg-surface-container-low border rounded-lg px-md py-sm text-body-sm font-mono focus:ring-2 focus:ring-primary', errors.kubeconfig ? 'border-error' : 'border-outline-variant']" placeholder="apiVersion: v1&#10;kind: Config&#10;..." @input="clearError('kubeconfig')"></textarea>
    ...
    <div v-if="form.authMethod === 'basic'" class="flex flex-col gap-sm">
      ...
      <div class="grid grid-cols-2 gap-sm">
        <div>
          <label class="text-body-xs text-on-surface-variant block mb-xs">{{ t('admin.clusters.username') }} <span class="text-error">*</span></label>
          <input v-model="form.username" data-testid="cluster-form-username" :class="['w-full bg-surface-container-low border rounded-lg px-md py-sm text-body-sm font-mono', errors.username ? 'border-error' : 'border-outline-variant']" @input="clearError('username')" />
