...
async function loadDir(path) {
  ...
  } catch (e) { error.value = e.message || '读取目录失败'; entries.value = [] }
...
async function openEntry(e) {
  ...
  try {
    ...
    editing.value = false
  } catch (err) { error.value = err.message || '读取失败' }
  finally { loading.value = false }
}
...
async function download() {
  ...
  } catch (e) { notify('error', e.message || '下载失败') }
}
...
async function onUpload(e) {
  ...
  try {
    ...
    notify('success', `已上传 ${f.name}（${f.size} 字节）`)
    loadDir(cwd.value)
  } catch (err) { notify('error', err.message || '上传失败') }
...
<template>
  <div class="flex flex-col h-full min-h-0">
    ...
    <div class="flex items-center gap-xs pb-sm border-b border-outline-variant/40 shrink-0">
      <button @click="goUp" :disabled="cwd === '/' || !!file" class="p-1 rounded-md text-on-surface-variant hover:bg-surface-container disabled:opacity-30" title="上一级"><span class="material-symbols-outlined text-base">arrow_upward</span></button>
      <button @click="loadDir(cwd)" :disabled="!!file" class="p-1 rounded-md text-on-surface-variant hover:bg-surface-container disabled:opacity-30" title="刷新"><span class="material-symbols-outlined text-base" :class="loading ? 'animate-spin' : ''">refresh</span></button>
      ...
      <button v-if="!file" @click="pickUpload" class="flex items-center gap-0.5 px-sm py-1 rounded-md bg-primary/10 text-primary text-xs hover:bg-primary/20 transition-colors shrink-0" title="上传到当前目录">
        <span class="material-symbols-outlined text-sm">upload</span>上传
      </button>
    </div>
    ...
    <div class="flex-1 overflow-auto mt-sm min-h-0">
      ...
      <div v-else-if="loading" class="py-md text-center text-body-sm text-on-surface-variant"><span class="material-symbols-outlined animate-spin inline-block">progress_activity</span> 加载中…</div>

      <!-- 文件查看/编辑 -->
      <template v-else-if="file">
        <div class="flex items-center gap-xs mb-sm sticky top-0 bg-surface-container-lowest pb-sm">
          <button @click="file = null; editing = false" class="flex items-center gap-0.5 text-xs text-primary hover:underline"><span class="material-symbols-outlined text-sm">arrow_back</span>返回列表</button>
          <span class="font-mono text-xs text-on-surface truncate flex-1" :title="file.path">{{ file.name }}</span>
          <span v-if="file.binary" class="text-[10px] px-1 rounded bg-surface-container text-on-surface-variant">二进制</span>
          <span v-else-if="file.truncated" class="text-[10px] px-1 rounded bg-tertiary-container/20 text-tertiary-container">已截断</span>
          <button @click="download" class="p-1 rounded-md text-on-surface-variant hover:text-primary hover:bg-primary/10" title="下载"><span class="material-symbols-outlined text-base">download</span></button>
          <template v-if="editable">
            <button v-if="!editing" @click="startEdit" class="flex items-center gap-0.5 px-sm py-1 rounded-md bg-primary/10 text-primary text-xs hover:bg-primary/20" title="编辑"><span class="material-symbols-outlined text-sm">edit</span>编辑</button>
            <template v-else>
              <button @click="cancelEdit" class="px-sm py-1 rounded-md border border-outline-variant text-xs text-on-surface hover:bg-surface-container">取消</button>
              <button @click="saveEdit" :disabled="saving" class="flex items-center gap-0.5 px-sm py-1 rounded-md bg-primary text-on-primary text-xs font-semibold hover:opacity-90 disabled:opacity-50"><span class="material-symbols-outlined text-sm">{{ saving ? 'progress_activity' : 'save' }}</span>保存</button>
            </template>
          </template>
        </div>
...
…（内容过长已截断，完整内容请下载）</span></pre>
      ...
      <!-- 目录列表 -->
      <template v-else>
        <p v-if="!entries.length" class="py-md text-center text-body-sm text-on-surface-variant/60">空目录</p>
