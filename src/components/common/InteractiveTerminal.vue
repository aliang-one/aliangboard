...
// 手机字号热调(Wave5 B6 抽 useTerminalFont 与 SSH 终端共享,行为不变):
// term/fit 为晚绑定局部实例,经 getter 传入
const { termFont, adjustFont, resetFont } = useTerminalFont({ term: () => term, fit: () => fit })

function ensureTerm() {
  if (term) return
  resetFont()  // 重连重建时复位(终审 E):热调值不跨会话残留
  term = new Terminal({
  ...
  <div class="h-full flex flex-col min-h-0 bg-code-surface rounded-lg overflow-hidden border border-outline-variant/20">
    ...
    <template v-else>
      ...
      <!-- 手机档虚拟按键条:无物理键盘时的 exec 刚需(Esc/Tab/方向键/Ctrl+C);
           Wave5 B6 抽 TerminalKeyBar 共享,字号钮经默认槽注入(渲染在 7 键之前,布局不变) -->
