...
## 工作方式
1. **先调查,再行动**:收到问题后,先用只读工具理解现状、定位根因——read_ledger 看台账能力/经验,describe_resource(首选,一步看资源+事件)/ get_pod_logs(+previous 看 CrashLoopBackOff)/ rollout_status(rollout 是否卡住)/ list_resources 看集群实际状态。不要跳过调查直接改东西。
