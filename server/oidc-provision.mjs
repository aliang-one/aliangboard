...
//   - OIDC 用户:authProvider='oidc'、passwordHash=哨兵哈希(hashPassword(randomUUID()),口令无人知晓
//     → 本地密码登录恒失败;生产 DDL 该列 NOT NULL,存 NULL 会炸——审 1 Critical)
...
import { hashPassword as defaultHashPassword } from './admin-seed.mjs' // 平台同格式 scrypt 哈希(纯函数,无副作用)
...
// { error:'usernameTaken' };③否则 INSERT(authProvider='oidc', passwordHash=哨兵, role='user')。
// deps 可注入 hashPassword/randomUUID(测试/生产接线点);缺省 = admin-seed 同款 + node:crypto。
// 注:disabled 不在此拦(建户 ≠ 放行)——Task 4 finishOidcSession 必须拒绝 disabled 用户。
export function upsertOidcUser(db, { issuer, sub, username, displayName }, { hashPassword = defaultHashPassword, randomUUID = defaultRandomUUID } = {}) {
  ...
  const id = randomUUID()
  const passwordHash = hashPassword(randomUUID()) // 哨兵:满足 NOT NULL 且本地登录恒失败
  db.prepare(`INSERT INTO platform_users (id, username, passwordHash, role, displayName, createdAt, authProvider, oidcSubject)
    VALUES (?, ?, ?, 'user', ?, ?, 'oidc', ?)`).run(id, username, passwordHash, displayName ?? null, Date.now(), subject)
...
// 重复组名先去重(真 IdP 可重复发同组;不去重会撞 group_members 主键炸整次登录)。
// 本地用户(或用户不存在)→ no-op { created:0 }。
export function syncGroupsFromClaims(db, userId, groupNames, { randomUUID = defaultRandomUUID } = {}) {
  ...
  try {
    ...
    for (const name of names) memberInsert.run(groupById.get(name).id, userId, now)
    db.exec('COMMIT')
    return { created }
  } catch (e) {
    db.exec('ROLLBACK')
