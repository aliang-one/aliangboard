  ...
  async function handle(req, res, url) {
    ...
    if (url.pathname.match(/^\/api\/workbench\/conversations\/[^/]+\/messages$/) && req.method === 'POST') {
      ...
      try {
        ...
        // Phase D(Task 7)+W2 审计 P0-①:六触发面 entitlement 以项目 owner 为准(ownerEntitled,
        // admin 代触发不得越过 owner 失权);owner 链已由上方 resolveConvProject 先行(authz-entitlement-07)。
        if (!ownerEntitled(project)) { sendJson(res, 403, { message: msg(req, 'wbp.clusterForbidden') }); return true }
        const cfg = getLlmConfig()
        if (!cfg.baseURL || !cfg.model) { sendJson(res, 400, { message: msg(req, 'wbc.llmNotConfigured') }); return true }
    ...
    if (url.pathname.match(/^\/api\/workbench\/conversations\/[^/]+\/regenerate$/) && req.method === 'POST') {
      ...
      try {
        ...
        // Phase D(Task 7)+W2 审计 P0-①:六触发面 entitlement 以项目 owner 为准(ownerEntitled;
        // owner 链已在上方 resolveConvProject 先行,authz-entitlement-07 busy 后置)。
        if (!ownerEntitled(project)) { sendJson(res, 403, { message: msg(req, 'wbp.clusterForbidden') }); return true }
        const cfg = getLlmConfig()
        if (!cfg.baseURL || !cfg.model) { sendJson(res, 400, { message: msg(req, 'wbc.llmNotConfigured') }); return true }
    ...
    if (url.pathname.match(/^\/api\/workbench\/conversations\/[^/]+\/approve$/) && req.method === 'POST') {
      ...
      try {
        ...
        // 用户下次仍人审,fail-closed)。payload 原样保留。CAS 命中后才读 convForGate.pendingApproval
        // (CAS 只翻 status,载荷未动)——迟到/重复审批走不进这里,remember 零副作用。
        let rememberInput = {}
