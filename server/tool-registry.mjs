...
const WB = [
  ...
  { name: 'http_request', requiresApproval: true,
    description: '对凭据声明的 base_url 发起 HTTP 请求(仅 GET):api_token 由平台服务端注入 Authorization 头,token 明文对你不可见。目标仅限该凭据 base_url 的 origin。',
    promptHint: '调用 Web API 时用(credential=凭据名,见系统提示清单 ✓标记;path=相对路径)。token 由平台注入,不要尝试获取或传递它。',
    inputSchema: { type: 'object', properties: {
      ...
      method: { type: 'string', description: '固定 GET(写方法后续版本)' },
